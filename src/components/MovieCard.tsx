    ...
    <div className="hover:scale-105 image-container transition-all ease-in-out w-full">
      ...
      <div className="hover-text w-full">
        <div className='font-hanalei gap-y-1 text-lg flex flex-col justify-center items-center w-full'>
          ...
          <div className='text-[#1EFF00]'>{parseInt(video.price)} PTT</div>
...
const MovieCard = ({ video }: any) => {
  ...
  // if(video[0] == 4){
  //   video[4] = "bafkreicqtwjgpk2l6cy36blhmwxex4bav3r23twpbq6ipk2h5y5bc2iaju"
  // }
  ...
  return (
    <div className="hover:scale-105 image-container transition-all ease-in-out w-full">
      <img
        src={`https://turquoise-certain-fox-148.mypinata.cloud/ipfs/${video[3]}`}
        className='w-full transition-all ease-in-out h-[321px] object-cover rounded-lg'
        alt=""
      />
      <div className="hover-text w-full">
        <div className='font-hanalei gap-y-1 text-lg flex flex-col justify-center items-center w-full'>
          <div className='text-2xl'>{video[1]}</div>
          <div className='text-[#1EFF00]'>{parseInt(video[4])} PTT</div>
          <div className='text-sm'>{video[2].slice(0, 100) + "..."}</div>
          <div className='happy-monkey-regular text-xl w-full flex flex-col gap-y-1'>
            <button className='bg-white hover:bg-gray-300 active:bg-gray-50 px-3 py-1 text-black rounded-lg' onClick={() => navigate(`/payment/${parseInt(video[0])}`)}>Buy</button>
            <button className='bg-white hover:bg-gray-300 active:bg-gray-50 px-3 py-1 text-black rounded-lg' onClick={() => navigate(`/trailerPlayer/${parseInt(video[0])}`)}>Trailer</button>
          </div>
        </div>
      </div>
    </div>
